...
/** Non-empty AND existing — task writes against a typo'd projectId create orphans. */
function validProject(projectId: string): boolean {
  return !!projectId && !!getProject(projectId);
}

// GET /api/tasks/prime — AI-optimised context dump for agent startup
router.get('/api/tasks/prime', (c) => {
  const projectId = c.req.query('projectId') ?? '';
  if (!validProject(projectId)) return c.text('projectId missing or unknown', 400);
...
router.post('/api/tasks/create', async (c) => {
  ...
  if (!validProject(projectId)) return c.json({ error: 'projectId missing or unknown' }, 400);
